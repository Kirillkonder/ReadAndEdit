...
class UserRepository implements IUserRepository {
  ...
  public async create(userData: CreateUserDto): Promise<void> {
    ...
    if (!userExists) {
      ...
      await database.run(
        `INSERT INTO users (userId, firstName, lastName, username, createdAt, subscriptionActive, subscriptionTier, isAdmin) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [userData.userId, userData.firstName, userData.lastName || null, userData.username || null, Date.now(), 0, 'free', isAdmin]
      );
    ...
    try {
      ...
      if (ctx.businessConnection && ctx.businessConnection.user_chat_id) {
        await ctx.api.sendMessage(
          ...
          `🥳 Бизнес-соединение с ID <code>${businessConnectionId}</code> успешно установлено.`,
          { parse_mode: "HTML" }
        );
      }
    } catch (error) {
    ...
    try {
      ...
      const notificationMessage = await ctx.api.sendMessage(
        ...
        dedent`
          ...
          👤 <b>Пользователь:</b> ${deletedMessage.senderName}
          📅 <b>Время отправки:</b> ${formatDate(deletedMessage.sentAt)}
          
          <i>Нажмите "Посмотреть" чтобы увидеть содержимое сообщения</i>
        `,
        {
          parse_mode: "HTML",
          reply_markup: {
            inline_keyboard: [
              [{ 
                text: "👀 Посмотреть", 
                callback_data: `show_deleted:${messageId}` 
              }]
            ]
          }
        }
        ...
        if (newMessageText && oldMessage) {
          ...
          await ctx.api.sendMessage(
            ...
            dedent`
              ...
              📅 <b>Время отправки:</b> ${formatDate(oldMessage.sentAt)}
              
              <i>Нажмите "Посмотреть" чтобы увидеть изменения</i>
            `,
            {
              parse_mode: "HTML",
              reply_markup: {
                inline_keyboard: [
                  [{ 
                    text: "👀 Посмотреть", 
                    callback_data: `show_edited:${message_id}` 
                  }]
                ]
              }
            }
...
async function handleCallbackQuery(ctx: Context) {
  ...
  try {
    ...
    // Обработка обычных callback-ов для сообщений
    const [action, messageId] = data.split(':');
    const messagesCollection = new MessagesRepository();
    const message = await messagesCollection.getById(parseInt(messageId));

    if (!message) {
      await ctx.answerCallbackQuery("Сообщение не найдено");
      return;
    }
    ...
    if (action === 'show_deleted') {
      ...
      if (message.media) {
        text = dedent`
          📷 <b>Удаленное сообщение с медиа</b>
          
          👤 <b>Пользователь:</b> <a href="t.me/${message.senderUsername || "whocencer"}">${message.senderName}</a>
          🆔 <b>ID:</b> <code>${message.senderId}</code>
          📅 <b>Отправлено:</b> ${formatDate(message.sentAt)}
          🗑️ <b>Удалено:</b> ${formatDate(message.deletedAt || Date.now())}
          
          📝 <b>Текст сообщения:</b>
          <blockquote>${message.text || "Без текста"}</blockquote>
        `;
      } else {
        text = dedent`
    ...
    } else if (action === 'show_edited') {
      const lastEdit = message.editedMessages[message.editedMessages.length - 1];
      const text = dedent`
        ...
        🆔 <b>ID:</b> <code>${message.senderId}</code>
        📅 <b>Отправлено:</b> ${formatDate(message.sentAt)}
        ✏️ <b>Отредактировано:</b> ${formatDate(message.editedAt || Date.now())}
        
        📝 <b>Было:</b>
        <blockquote>${lastEdit?.oldMessageText || message.text}</blockquote>
  ...
  private async startCommandHandler(ctx: Context) {
    try {
      if (ctx.from) {
        ...
        // Активируем вечную подписку для админа
